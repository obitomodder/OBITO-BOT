...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Obito\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5517981992106:+55 47992091566\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan esse codigo QR ai'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*TCHAU @${num.split('@')[0]} -1 CORNO*`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*FALHOU, TENTA NOVAMENTE AI!*',
				Iv: '*LINK INVALIDO, CONFIRA O QUE ESTÁ ERRADO E MANDE CERTO!*'
				},
				only: {
					group: '*[❗] ESSE COMANDO SÓ PODE SER USADO EM GRUPOS*',
					premium: '*[❗] ESSE PEDIDO É SO PARA *USUÁRIOS PREMIUMS*',
					mod: '*[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD OBITO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '*[❗] OBITO? ESSE É UM RECURSO SOMENTE PARA O OBITO*',
					ownerB: '*[❗] OBITO? ESSE É UM RECURSO SOMENTE PARA O OBITO*',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Obito. Registre-se para fazer amizade com o bot Obito por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Obito|17\n\n──「 Obito BOT 」──`,
					admin: '*[❗] ESSE COMANDO SÓ PODE SER USADO POR ADMINISTRADORES DO GRUPO!*',
					Badmin: '*[❗] ESSE COMANDO SÓ PODE SER USADO QUANDO O BOT É ADM DO GRUPO!*',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5517981992106@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5517981992106@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5517981992106@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5517981992106@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5517981992106@s.whatsapp.net","12135292197@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text);
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DETECTADO ${sender.split("@")[0]} VOCÊ SERA EXPULSO DO GRUPO EM 5 SEGUNDOS!*`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("calma la mano, xinga a mae não")
	}
    ...
    client.updatePresence(from, Presence.composing)
    tujuh = fs.readFileSync('./assets/banido.mp3');
          client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
  }
      if (messagesC.includes("ednaldo banido")){
  client.updatePresence(from, Presence.composing)
  tujuh = fs.readFileSync('./assets/edbanido.mp3');
        client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			switch(command) {
				...
				case 'ramaljadian':
                   ...
                   case 'gcpf':
                    ...
                    break
				case 'ytmp4':
					if (args.length < 1) return reply('*CADE O URL? TEM QUE ESTAR NA FRENTE DO COMANDO*')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://st4rz.herokuapp.com/api/ytv2?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*TITULO* : ${anu.title}\n\n *PARA TRANSFORMAR O VIDEO EM MUSICA, MARQUE ELE COM .tomp3*`
...
break
		   ...
		   hasil = `*VEJA O QUANTO VC É GAY* ${gay}\n\n\n*PORCENTAGEM GAY: ${anu.persen}%*`
		   reply(hasil)
		   await limitAdd(sender)
					break
                case 'nangis':
...
break
				...
				case 'testime':
					...
					break
		        case 'gay':
	            	if (args.length < 1) return reply('*MARCA QUEM VOCÊ QUER QUE VEJA A %*')
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39','99','1','10']
					const kl = ti[Math.floor(Math.random() * ti.length)]
					client.sendMessage(from, '*O QUANTO VOCÊ É GAY**'+rate+'*\n\n*SUA PORCENTAGEM GAY: '+ kl+'%*', text, { quoted: mek })
					break
                  case 'timer':
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS Obito BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Obito domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*VIDEO ENCONTRADO!*\n*TÍTULO:* ${anu.result.title}\n*URL:* ${anu.result.source}\n*TAMANHO:* ${anu.result.size}\n\n*NÃO SPAMME ESSE COMANDO NO BOT*`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
          ...
          reply('Espere..')
				...
				case 'f':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`*MARQUE A MENSAGEM NOVAMENTE COM .f OU REENVIE ELA COM .f NA LEGENDA*`)
					}
					break
				case 'tts':
				...
				case 'walpaperanime':
					...
					break
					case 'dado':
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Obito ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Obito ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *Obito* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Obito_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5517981992106/*╰──────*「 * Obito* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO Obito BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod':
          ...
          buffer = await getBuffer(`https://fotosdemulheresnuas.net/wp-content/uploads/2018/12/Mia-Khalifa-fotos-5.jpg`)
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*PRONTO, NO FUTURO TERÁ MAIS FOTOS!*'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Então quer ver loli? Analise'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta querendo ver HENTAI amigão?'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia fml ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde fml 😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*CANAL DO OBITO:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
			...
			reply(mess.wait)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*[ AVISO PARA TODOS! ]*\n\n${body.slice(4)}`})
						}
						reply('Transmissão enviada com sucesso')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*[ AVISO PARA TODOS! ]*\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada com sucesso')
					}
					break
